...
class CoinTicketManager(object):
    ...
    def add_coins(self):
        ...
        print "coins_record",coins_record    
        coins_record.coins = coins_record.coins + 10
        print "in add_coins",coins_record.coins
        coins_record.save()
          
    
    def update(self):
        print "in update coins manager"
        if self._device:
            self._allocated = 0
        
                    
         # Process Checked v.s. Unchecked
        print "sahhhhhhhhhhhhhhhhhhhhhhhhhj"
        print self._ticket
        if not CoinTicketTransaction.objects.filter(ticket=self._ticket).exists():
            print "in coins filter"
            transaction = CoinTransaction(wallet=self._wallet)
            transaction.save()
            print "in jasjjdskjkkkkkkkkkkkkk"
            try:
                print 11111111111111111174747774
                self.add_coins()
            except:
                pass
...
class CoinTicketManager1(object):
	...
	from yoolotto.lottery.models import LotteryTicketAvailable
        ...
        if count1 > self._countt:
            email_coins_record.coins = email_coins_record.coins - 0.2
