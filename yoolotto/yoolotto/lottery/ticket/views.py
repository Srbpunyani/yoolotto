...
class Play_Game(View):
    ...
    def post(self, request):
        ...
        if ( request.META["HTTP_YOO_APP_VERSION"] in ['3.0.2','4.2.2'] and data[0]['state'] == 'TX' ) or not request.yoo['new_version']:
            ...
            # Return Tickets
            result = map(lambda x: x.representation(request.META['HTTP_YOO_EMAIL_ID'],allocated=allocated), tickets.values())
    ...
    #version = request.META["HTTP_YOO_APP_VERSION"]
	    ...
	    ticket_user = ticket_info.user
                    ...
                    for line in lines:
                        ...
                        try:
                            if data1["drawTimeTo"]:
                                ...
                                req_json = {"gameType": data["gametypeid"], "state": data["state"], "drawingDate": draw_done.date.strftime('%Y-%m-%d'), "Consecutive": data["Consecutive"], "multiDraw": data["multiCheck"], "drawTime": data["drawTime"], "RetailerNo": data["RetailerNo"], "multiDrawEndDate": data1["multiDrawEndDate"]}
                        except:
                            if data1["drawTimeTo"]:
                    ...
                    for line in lines:
                        ...
                        try: 
                            if data1["drawTimeTo"]:   
                                ...
                                req_json = {"gameType": data["gametypeid"], "state": data["state"], "drawingDate": data["drawdate"], "Consecutive": data["Consecutive"], "multiDraw": data["multiCheck"], "drawTime": data["drawTime"], "RetailerNo": data["RetailerNo"], "multiDrawEndDate": data1["multiDrawEndDate"]}
                        except:
                            if data1["drawTimeTo"]:
                  ...
                  for line in lines1:
                      ...
                      try:
                        if data1["drawTimeTo"]:
                            ...
                            req_json = {"gameType": data["gametypeid"], "state": data["state"], "drawingDate": draw.date.strftime('%Y-%m-%d'), "Consecutive": data["Consecutive"], "multiDraw": data["multiCheck"], "drawTime": data["drawTime"], "RetailerNo": data["RetailerNo"], "multiDrawEndDate": data1["multiDrawEndDate"]}
                      except:
                        if data1["drawTimeTo"]:
            ...
            if version <= "4.2.6":
                ...
                result = map(lambda x: x.representation_old(allocated=allocated), tickets.values())
            else:
...
class Ticket(View):
    ...
    def _representation(self, tickets,version):
        if version <= "4.2.6":
            ...
            if not isinstance(tickets, list):            
                return tickets.representation_old()
              
            return map(lambda x: x.representation_old(representations.TicketRepresentationShortList_old), tickets)
...
class YooTicket(View):
    ...
    def get(self, request):
        ...
        if game_type is None and draw_id is None:        
            #tickets = list(user.tickets.filter(division_id=division.id).exclude(winnings=None).order_by("-draw__date"))
            tickets = user.tickets.filter(division_id=division.id,deleted=False).order_by("-draw__date") | user.tickets.filter(division_id=division.id,deleted=False).exclude(draw__result=None).order_by("-draw__date")
        ...
        #tickets = [ticket for ticket in tickets if ticket.id not in unavailable_tickets]
            ...
            #tickets = user.tickets.filter(draw__component__parent=game,division_id=division.id).exclude(winnings=None).order_by("-draw__date")
            tickets = user.tickets.filter(draw__component__parent=game,division_id=division.id,fantasy = False,deleted=False).exclude(winnings=None).order_by("-draw__date") | user.tickets.filter(draw__component__parent=game,division_id=division.id,fantasy = False,deleted=False).exclude(draw__result = None).order_by("-draw__date")
            #tickets = list(tickets)
        print "the tickets are...", tickets
        fantasy_tickets = user.tickets.filter(fantasy= True,deleted=False).exclude(winnings=None).order_by("-draw__date") | user.tickets.filter(fantasy = True,deleted=False).exclude(draw__result = None).order_by("-draw__date")
        print fantasy_tickets
        #tickets = tickets.extend(fantasy_tickets)
        tickets = list(set(tickets | fantasy_tickets))
        print "main tickets",tickets
    ...
    def post(self, request, _id):
        ...
        if version <="4.2.6":
           _result = ticket.representation1() 
            ...
            _result = ticket.representation_old()
        else:
...
class enumeration_data(View):
    ...
    def get(self,request):
        ...
        dollar_amount = total_coins/100

        return {"numbers_money":NUMBERS_MONEY,"total_coins":total_coins,"dollar_amount":dollar_amount,"dollar_exchange_rate":"100"}
