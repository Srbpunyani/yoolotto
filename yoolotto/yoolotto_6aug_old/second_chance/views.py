...
from yoolotto.second_chance.models import FiberCoins, DwollaUserInfo, DwollaTransactionInfo
...
class Det_Data(View):
    def get(self,request):
        ...
        a = 48611
        b = 48617
        ticket_record = LotteryTicket.objects.filter(id__range = (a,b))
        #ticket_record = LotteryTicket.objects.filter(id = 48608)
        for tckt in ticket_record:
            ticket_id = tckt.id
            ticket_available = LotteryTicketAvailable.objects.get(ticket_id = ticket_id)
            game_type = int(ticket_available.gameType)
            DET_GAMES = {11: 1,1: 1,13: 2,0: 2,2: 3,3: 5,4: 6,5: 7,6: 4,7: 8,}
            valid_data_entry_game_type = DET_GAMES[game_type]
            data = {"StateId":tckt.division_id,"DrawDate":str(tckt.draw.date),"GameTypeId":valid_data_entry_game_type,"PlayId":ticket_available}
            print "data",data
            url = 'http://52.6.245.123/DetService4.0/DataEntryTool.WcfService.TicketService.svc/AddTicket'
