...
def send_data_to_det(success,ticket,play,game_type,image_data,state):
    if success:
...
class Play_Game(View):
	...
	image_data = bytearray(eval(data[0]['image']))
        ...
        if request.yoo['new_version']:
            ...
            thread.start_new_thread( send_data_to_det, (success,ticket,play,game_type,image_data,data[0]['state']) )
